...
Gfonction<-function(z){
  ...
  B[-vec,]<-A
  C<-B[-c(3,9,14,20),]# For each variable, delete one modality POV : sinon les contraintes ne sont pas indépendantes
  return(C)
}
...
view(Moment_mat) # POV : ceci permet de voir l'intérêt de cette matrice 
...
finterp2<-function(x,z){ # POV interpolation linéaire selon l'équation 20 mais pour une modalité
  A<-c()
  B<-c()
  C<-c()
  D<-c()
...
ech_compo<-finterp2(x=decile_compo,z=vec_all_incomes) #  POV : decile_compo est le tableau  des déciles pour chaque modalité de l'attribut composition des familles, la sortie est le tableau des probabilités du revenu I < aux 190 modalités du revenu pour chaque modalité de l'attribut combo
...
# POV  calcul des dénominaeurs de l'équation 22
# ceci ne sert qu'à normaliser,les 4 valeurs devraient être identiques
# ce n'est pas le cas et cela demande à être travaillé 
...
l1 <- list(rep(1,length(vec_all_incomes))) # POV : contrainte naturlle

eta_total<-c(l1,constraint_statut,constraint_age,constraint_size,constraint_composition)

eta_final<-eta_total[c(-3,-9,-14,-20)] # For each variable, delete one modality, POV afin de viser le rang plein 
...
I<-A$pivot[1:A$rank] # matrice de permutation 
...
for (i in 1:length(vec_all_incomes)){
  ...
  Iii=Ii 
  repeat{ # POV : boucle tant que le système n'est pas compatible, on enlève alors une contriante
    Id=diag(eta_c[Iii,i])
